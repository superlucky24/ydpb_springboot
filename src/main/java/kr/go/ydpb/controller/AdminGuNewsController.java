...
@Controller
@AllArgsConstructor
@RequestMapping("/admin/gunews/*")
public class AdminGuNewsController {
    ...
    public String view(@RequestParam("gnewsId") Long gnewsId,
        ...
        return "admin/admin_gunews_view";
    }

    // 글작성 화면
    @GetMapping("write")
    public String writeFrom(@ModelAttribute("cri") Criteria cri) {
        return "admin/admin_gunews_write";
    }

    // 글작성 실행
    @PostMapping("write")
    public String write(GuNewsVO board,
                        @ModelAttribute("cri") Criteria cri,
        ...
        if(board.getGnewsId() > 0) {
            ...
            return "redirect:/admin/gunews/write";
        }
    }

    // 글수정 화면
    @GetMapping("update")
    public String updateFrom(@RequestParam("gnewsId") Long gnewsId,
                             @ModelAttribute("cri") Criteria cri,
                             Model model) {
        model.addAttribute("board", service.getBoard(gnewsId));
        return "admin/admin_gunews_update";
    }

    // 글수정 실행
    @PostMapping("update")
    public String update(@ModelAttribute("board") GuNewsVO board,
                         @ModelAttribute("cri") Criteria cri,
                         @RequestParam(value = "file_1", required = false) MultipartFile file1,
                         @RequestParam(value = "file_2", required = false) MultipartFile file2,
                         @RequestParam(value = "file_text_1", required = false) String fileText1,
        ...
        rttr.addAttribute("gnewsId", board.getGnewsId());
        rttr.addAttribute("pageNum", cri.getPageNum());
        rttr.addAttribute("amount", cri.getAmount());
        rttr.addAttribute("searchType", cri.getSearchType());
        rttr.addAttribute("searchKeyword", cri.getSearchKeyword());
        return "redirect:/admin/gunews/view";
    }

    // 글삭제
    @PostMapping("delete")
    public String delete(Long gnewsId,
                         @ModelAttribute("cri") Criteria cri,
                         RedirectAttributes rttr) {
        int result = service.deleteBoard(gnewsId);
        if(result > 0) {
