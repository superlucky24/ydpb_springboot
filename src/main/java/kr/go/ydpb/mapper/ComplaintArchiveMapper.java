...
//    public int getAllCount(Criteria cri); // 모든 민원 갯수 파악
    //public List<ComplaintVO> getAllComplaint(int start, int end);
     ComplaintVO getOneComplaintArc(int comId); // 하나의 민원
     List<ComplaintArchiveVO> getAllComplaintArc();
     int insertComplaintArc(ComplaintVO cvo); // 민원 아카이브 등록
     void updateComplaintArc(ComplaintVO cvo); //아카이브 관리자 민원 수정(답변)
